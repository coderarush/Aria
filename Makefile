...
SIGN_ID ?= $(shell security find-identity -p codesigning 2>/dev/null | grep -q "Friday Self-Signed" && echo "Friday Self-Signed")
ifeq ($(strip $(SIGN_ID)),)
SIGN_ID := -
endif
...
cert:
	@if security find-identity -v -p codesigning | grep -q "Friday Self-Signed"; then \
		...
		openssl pkcs12 -export -legacy -inkey /tmp/friday.key -in /tmp/friday.crt -out /tmp/friday.p12 -passout pass:friday 2>/dev/null; \
		security import /tmp/friday.p12 -k ~/Library/Keychains/login.keychain-db -P friday -A -T /usr/bin/codesign; \
